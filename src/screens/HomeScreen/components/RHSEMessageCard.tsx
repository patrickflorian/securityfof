...
const RHSEMessageCard = (props: any) => {
    ...
    const editMessage = (message: string)=>{
        navigation.navigate(routenames.MESSAGE_FORM, {message: message})
    }

    return (<Card theme={theme} style={{ marginBottom: 15, marginHorizontal: 5 }}>
        <Card.Title title="Le RHSE" left={LeftContent} right={props => user?.is_manager && <IconButton {...props} icon="comment-edit-outline" onPress={()=>editMessage(message)}/>} />
        <Card.Content>
